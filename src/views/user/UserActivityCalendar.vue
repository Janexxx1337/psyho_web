<template>
  <div>
    <el-calendar v-model="currentDate" :locale="ruLocale">
...
<script setup lang="ts">
  ...
  const addActivity = () => {
    ...
    userActivitiesStore.addActivity(
      dayjs(selectedDate.value).toDate(),
      newActivity.value
    );
    selectedActivities.value = [
      ...userActivitiesStore.activities[selectedDate.value],
    ];
    newActivity.value = '';
  ...
  const dialogTitle = computed(
    () => `Активность за ${formatDate(selectedDate.value)}`
  );
</script>
